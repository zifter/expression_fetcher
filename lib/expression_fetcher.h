...
public:
    ...
    {
        std::stringstream ss;
        ss << '[' << get_left_string() << ']' << " is " << "true";
        return ss.str();
    }

    std::string get_expression(const Fetcher &right) const
    {
        std::stringstream ss;
        ss << '[' << get_left_string() << right.get_right_string() << ']' << " is " << "true";
        return ss.str();
    }
...
#define EVAL_CONDITION(expr) \
    [&](){ \
        ...
        { \
            Fetcher right; \
            expr % right; \
            return left.get_expression(right); \
        } \
        else \
        { \
