...
{
    Check(DUMP_CONDITION("test" == nullptr), "\"test\" == nullptr");
}

FETCHER_TEST(test_string_equal_nullptr)
{
    Check(DUMP_CONDITION(std::string("test") == nullptr), "\"test\" == nullptr");
}
...
FETCHER_TEST(test_less)
{
    int valA = 42;
    float valB = 30.4f;
    Check(DUMP_CONDITION(valA < valB), "42 < 30.4");
}
...
FETCHER_TEST(test_more)
{
    int valA = 55;
    float valB = 35.4f;
    Check(DUMP_CONDITION(valA > valB), "55 > 35.4");
}
...
FETCHER_TEST(test_and)
{
    bool valA = true;
    bool valB = false;
    Check(DUMP_CONDITION(valA && valB), "true && false");
}
...
FETCHER_TEST(test_not_equal)
{
    int valA = 12;
    int valB = 16;
    Check(DUMP_CONDITION(valA != valB), "12 != 16");
}
...
FETCHER_TEST(test_more_and_less)
{
    ...
    float valC = 22.22f;
    float valD = 11.11f;
    Check(DUMP_CONDITION(valA > valB && valC < valD), "50 > 33.3 && 22.22 < 11.11");
}
...
FETCHER_TEST(test_equal_and_equal)
{
    ...
    int valC = 22;
    int valD = 11;
    Check(DUMP_CONDITION(valA == valB && valC == valD), "50 == 33 && 22 == 11");
}
...
FETCHER_TEST(test_divide_or_divide)
{
    ...
    Check(DUMP_CONDITION(valA / valB && valC / valD), "50 / 33 && 2");
}
...
FETCHER_TEST(test_mult_or_mult)
{
    ...
    int valD = 0;
    // there is no chance to catch right part
    Check(DUMP_CONDITION(valA * valB && valC * valD), "50 * 33 && 0");
}
...
FETCHER_TEST(test_expression_with_2_stop_and)
{
    ...
    Check(DUMP_CONDITION(valA - valB && valC < valD && valE > valF), 
        "50 - 33 && false && 23 > 45");
}
